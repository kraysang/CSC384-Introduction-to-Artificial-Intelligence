...
def prop_FC(csp, newVar=None):
    ...
    for c in constraints:
        if c.get_n_unasgn() == 1:
            DOW, pruned_n = FC_Check(c)
            pruned = pruned + pruned_n
            if DOW:
                return False, pruned
    return True, pruned
...
def prop_GAC(csp, newVar=None):
    ...
    while len(GAC_queue) != 0:
        constraints = GAC_queue.pop(0)

        for v in constraints.get_scope():
            for d in v.cur_domain():
                if not constraints.has_support(v, d):
...
def ord_mrv(csp):
    ...
    return min(unasgn_vars, key=lambda v: v.cur_domain_size())
